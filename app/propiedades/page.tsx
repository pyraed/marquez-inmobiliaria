...
import { NOMBRE_INMOBILIARIA } from "../../lib/config";
...
export default async function PropiedadesPage({ searchParams }: Props) {
  ...
  // Construir query con filtros del servidor
  let query = supabase
    .from("propiedades")
  ...
  // Precio: filtrar por precio_valor si es numérico
  if (params.precioMin) {
  ...
  // Búsqueda de texto (título o ubicación)
  if (params.q) {
  ...
  const { data: propiedadesData } = await query;
  const propiedades = (propiedadesData as unknown as PropiedadCardType[]) || [];

  // Localidades únicas para el filtro (todas las publicadas)
  ...
  const localidades = [...new Set((localidadesData || []).map((p: { localidad: string }) => p.localidad).filter(Boolean))];
